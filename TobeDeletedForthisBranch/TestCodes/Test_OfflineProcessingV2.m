clear all
recprops = RecordingProperties();
signal = Signal(recprops);

%% Load Signal
data = signal.LoadSignal();
Fs=data.recProps.SamplingFreq;
sF=Fs;
numCh=data.recProps.NumChannels;
signal=data.signal;

%signal= signal+(max(signal(1,:))/10)* rand(1, length(signal));
%plot(signal(1,:))




%% Filter Signal

%Notch
N   = 6;  % Order
Fc1 = 49;  % First Cutoff Frequency
Fc2 = 51;  % Second Cutoff Frequency

[z,p,k] = butter(N/2, [Fc1 Fc2]/(Fs/2), 'stop');

[sos_var,g] = zp2sos(z, p, k);
Hd50        = dfilt.df2sos(sos_var, g);

signal = filter(Hd50,signal);


% Bandpass filter
N   = 4;     % Order
FC1=20;
FC2=400;

% Calculate the zpk values using the BUTTER function.
[z,p,k] = butter(N/2, [FC1 FC2]/(Fs/2));

[sos_var,g] = zp2sos(z, p, k);
Hd          = dfilt.df2sos(sos_var, g);
%signal      = filtfilt(sos_var,g,signal);
signal      = filter(Hd50,signal);



%% Split the data into the time windows

% Add tags to data
tags=data.tags;
[~,eventDataWithTags]=SignalTagger(signal,tags,numCh,Fs);


nM=length(unique(eventDataWithTags(numCh+1,:)));
%nM   =numMovements* (numMovements-1) ;        % Number of events
eventLen_Transition = 1.2;
eventLen_Static=2;
eventOffset_Transition=-0.2;
tWindow=0.2;
tOverlap=0.05;

nEventLen_Transition = round(eventLen_Transition * Fs);  % Samples per event
nEventLen_Static=round(eventLen_Static * Fs);  % Samples per event;
nWSamples=round(tWindow * Fs); % Samples corresponding Time window
nOverlap=round(tOverlap * Fs);   % Samples correcponding overlap
nNonOverlap = nWSamples - nOverlap;

nW_Transition= floor(nEventLen_Transition/(nWSamples-nOverlap) ) ;    %number of windows per event
nW_Static= floor(nEventLen_Static/(nWSamples-nOverlap) ) ;
% Check if final window fits actually
if nW_Transition * nNonOverlap + nOverlap > nEventLen_Transition
    nW_Transition = nW_Transition - 1;
end

%signal=signal';
%datawithtags=zeros(length(tags(1,:)),size(signal,1),size(signal,2));

i=1;
ns=1;
uniquetags=unique(eventDataWithTags(numCh+1,:),'stable');

% Separate events just 30 seconds for each event

NewTransitionData=[];
NewStaticData=[];
for s=1:length(eventDataWithTags)
    
    if i<=length(uniquetags) && eventDataWithTags(numCh+1,s)==uniquetags(i) && s>500
        nEventStart = s+ eventOffset_Transition * Fs;
        nEventEnd = nEventStart + nEventLen_Transition - 1;
        if uniquetags(i)>=10        %% It was a transition
            TransitionData= eventDataWithTags(1:numCh,nEventStart:nEventEnd);
            TransitionData(numCh+1,:)= uniquetags(i);
           % plot(TransitionData(numCh+1,:));
            NewTransitionData=[NewTransitionData, TransitionData ];
            
        else
            nEventStart = s+ 1000;
            nEventEnd = nEventStart + 2*Fs - 1;
            StaticData = eventDataWithTags(:,nEventStart:nEventEnd);
            %plot(StaticData(numCh+1,:));
            NewStaticData=[NewStaticData ,StaticData];
            
        end
        i=i+1;
    end
end

% Reshape transition data into 3 dimension
NewTransitionData=NewTransitionData';
numTransition=length(unique(NewTransitionData(:,numCh+1)));
Trans=unique(NewTransitionData(:,numCh+1));
nStart=0;
b=[];
for i=1:numTransition
    a=NewTransitionData(nStart+1:nStart+length(TransitionData),:);
    nStart=nStart+length(TransitionData);
    b=cat(3,a,b);
end
NewTransitionData=b;
% Reshape Steady data into 3 dimension
NewStaticData=NewStaticData';
%numStatic=length(unique(NewStaticData(:,numCh+1)));
numStatic=4;
Static=unique(NewStaticData(:,numCh+1));
nStart=0;
b=[];
for i=1:numStatic
    a=NewStaticData(nStart+1:nStart+length(StaticData),:);
    nStart=nStart+length(StaticData);
    b=cat(3,a,b);
end
NewStaticData=b;
% Separate windows of events in transition
windowedDataTransition = [];
for e = 1 : numTransition
    for i = 1 : nW_Transition
        iidx = 1 + (nNonOverlap * (i-1));
        eidx = nWSamples + (nNonOverlap *(i-1));
        windowedDataTransition(:,:,e,i) = NewTransitionData(iidx:eidx,:,e);
    end
end

% Separate windows of events in steaady state
windowedDataStatic = [];
for e = 1 : numStatic
    for i = 1 : nW_Static-1
        iidx = 1 + (nNonOverlap * (i-1));
        eidx = nWSamples + (nNonOverlap *(i-1));
        windowedDataStatic(:,:,e,i) = NewStaticData(iidx:eidx,:,e);
    end
end



%40,40,20 Transition
trSets_Transition  = round(.4*nW_Transition);
vSets_Transition   = round(.2*nW_Transition);
tSets_Transition   = nW_Transition - trSets_Transition - vSets_Transition; %floor(.4*nW);
trdata_Transition  = windowedDataTransition(:,:,:,1:trSets_Transition);
vdata_Transition   = windowedDataTransition(:,:,:,trSets_Transition+1:trSets_Transition+vSets_Transition);
tdata_Transition   = windowedDataTransition(:,:,:,trSets_Transition+vSets_Transition+1:trSets_Transition+vSets_Transition+tSets_Transition);


%Static
trSets_Static  = round(.4*nW_Transition);
vSets_Static    = round(.2*nW_Transition);
tSets_Static    = nW_Transition - trSets_Static  - vSets_Static ; %floor(.4*nW);
trdata_Static   = windowedDataTransition(:,:,:,1:trSets_Static );
vdata_Static    = windowedDataTransition(:,:,:,trSets_Static +1:trSets_Static +vSets_Static);
tdata_Static   = windowedDataTransition(:,:,:,trSets_Static +vSets_Static +1:trSets_Transition+vSets_Static +tSets_Static );
%% Feature Extraction Transition
labelI = 0;
for m = 1: numTransition
    for i = 1 :trSets_Transition
        trFeatures(i,m) = GetSigFeatures(trdata_Transition(:,1:numCh,m,i),sF);
        labelI = labelI + 1;
        trLables(labelI)=trdata_Transition(1,numCh+1,m,i);
    end
    
end

labelI = 0;
for m = 1: numTransition
    for i = 1 : vSets_Transition
        vFeatures(i,m) = GetSigFeatures(vdata_Transition(:,1:numCh,m,i),sF);
        labelI = labelI + 1;
        vLables(labelI)=vdata_Transition(1,numCh+1,m,i);
    end
end

labelI = 0;
for m = 1: numTransition
    for i = 1 : tSets_Transition
        tFeatures(i,m) = GetSigFeatures(tdata_Transition(:,1:numCh,m,i),sF);
        labelI = labelI + 1;
        tLables(labelI)=tdata_Transition(1,numCh+1,m,i);
    end
end

%% Get sets

tLables=tLables';
Ntrset = trSets_Transition * numTransition;
Nvset  = vSets_Transition  * numTransition;
Ntset  = tSets_Transition  * numTransition;

features = ["tmabs",'twl' ,'tzc', 'tslpch2' ];

trSet = zeros(Ntrset, length(features));
vSet  = zeros(Nvset , length(features));
tSet  = zeros(Ntset , length(features));
%
% trOut = zeros(Ntrset, nM);
% vOut  = zeros(Nvset , nM);
% tOut  = zeros(Ntset , nM);

% Stack window feature together in a continous signal instead of different
% dimensions
clear e

for eventIndex = 1 : numTransition
    e = eventIndex;
    % Training
    for r = 1 : trSets_Transition
        sidx = r + (trSets_Transition*(e-1));
        li = 1;
        for i = 1 : length(features)
            le = li - 1 + length(trFeatures(r,e).(features{i}));
            trSet(sidx,li:le) = trFeatures(r,e).(features{i}); % Get each feature per channel
            li = le + 1;
        end
        %trOut(sidx,j) = 1;
    end
    % Validation
    for r = 1 : vSets_Transition
        sidx = r + (vSets_Transition*(e-1));
        li = 1;
        for i = 1 : length(features)
            le = li - 1 + length(vFeatures(r,e).(features{i}));
            vSet(sidx,li:le) = vFeatures(r,e).(features{i});
            li = le + 1;
        end
        %   vOut(sidx,j) = 1;
    end
    % Test
    for r = 1 : tSets_Transition
        sidx = r + (tSets_Transition*(e-1));
        li = 1;
        for i = 1 : length(features)
            le = li - 1 + length(tFeatures(r,e).(features{i}));
            tSet(sidx,li:le) = tFeatures(r,e).(features{i});
            li = le + 1;
        end
        %tOut(sidx,j) = 1;
    end
end


%% Classification
%% Randomize data (if requested)???

%%%%%%%%%%Labels
%   trLables=trdata(:,numCh+1,1,1);
%   tLables=tdata(:,numCh+1,1,1);
%   vLables=vdata(:,numCh+1,1,1);

%Apply discriminat to vset9
dType='linear';
trLables=trLables';




try
    %[vLabelsPredicted,err,POSTERIOR,logp,coeff] = classify(vSet,trSet,trLables,dType);
    classifierModel = fitcdiscr(trSet, trLables, 'DiscrimType', 'pseudoLinear');
    vLabelsPredicted = predict(classifierModel, vSet);
catch e
    disp(e)
    errordlg(e.message);
    accVset = [];
    coeff = [];
    return;
end

numCorrect = sum(vLabelsPredicted == vLables');
accTotal = numCorrect / Nvset;
correctPrediction = zeros(Nvset,1);

C = confusionmat(vLabelsPredicted, vLables);
confusionchart(C);

%%%%%%%%%%%%%%%55
%What about fitcdiscr and predict

%%%%%%%%%%5555


%[tLabelsPredicted,err,POSTERIOR,logp,coeff] = classify(tSet,trSet,trLables,dType);
 classifierModel = fitcdiscr(trSet, trLables, 'DiscrimType', 'pseudoLinear');
 tLabelsPredicted = predict(classifierModel, tSet);
 save('classifierModel','classifierModel');
%D = confusionmat(tLabelsPredicted, tLables);
%cm = plotconfusion(tLables,tLabelsPredicted);
%figure();
%confusionchart(D);
return;


    
    %  x = NormalizeSet(tSet(i,:), patRec);
    %x = ApplyFeatureReduction(x, patRec);
    
    
    

    

   



